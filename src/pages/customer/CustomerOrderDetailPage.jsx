import { useLocation, useNavigate, useParams } from 'react-router-dom';

export default function CustomerOrderDetailPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const order = location.state && location.state.order;

  if (!order) {
    return (
      <div style={{ padding: 24 }}>
        <div style={{ maxWidth: 960, margin: '0 auto', background: '#fff', borderRadius: 16, padding: 24 }}>
          <p>ไม่พบข้อมูลคำสั่งซื้อ</p>
          <button
            type="button"
            onClick={() => navigate('/customer/orders')}
            style={{
              marginTop: 8,
              padding: '8px 16px',
              borderRadius: 999,
              border: '1px solid #e5e7eb',
              background: '#fff',
              cursor: 'pointer',
              fontSize: 13,
            }}
          >
            กลับไปหน้าคำสั่งซื้อของฉัน
          </button>
        </div>
      </div>
    );
  }

  const items = order.items || [];
  const totalText = typeof order.total === 'number'
    ? order.total.toLocaleString()
    : (parseFloat(order.total || 0) || 0).toLocaleString();
  const dateText = new Date(
    order.withdrawDate?.seconds
      ? order.withdrawDate.seconds * 1000
      : order.withdrawDate
  ).toLocaleDateString('th-TH');
  const status = order.shippingStatus || 'รอดำเนินการ';
  const statusColor =
    status === 'ส่งสำเร็จ'
      ? '#16A34A'
      : status === 'กำลังดำเนินการส่ง'
      ? '#2563EB'
      : '#F97316';

  const deliveryMethod = (order.deliveryMethod || 'shipping') === 'pickup' ? 'รับเอง' : 'จัดส่ง';

  return (
    <div style={{ padding: '30px', background: '#f0f4ff', minHeight: '100vh' }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        <button
          type="button"
          onClick={() => navigate('/customer/orders')}
          style={{
            padding: '6px 14px',
            borderRadius: 999,
            border: '1px solid #e5e7eb',
            background: '#fff',
            cursor: 'pointer',
            fontSize: 13,
            marginBottom: 14,
          }}
        >
          ← กลับไปหน้าคำสั่งซื้อของฉัน
        </button>

        <div
          style={{
            background:
              'linear-gradient(135deg, #1D4ED8 0%, #2563EB 40%, #1D9BF0 75%, #4F46E5 100%)',
            borderRadius: 24,
            padding: '18px 22px',
            marginBottom: 18,
            color: '#fff',
            display: 'flex',
            justifyContent: 'space-between',
            alignItems: 'center',
            boxShadow: '0 10px 26px rgba(15,23,42,0.32)',
          }}
        >
          <div>
            <div style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', opacity: 0.85 }}>ORDER DETAIL</div>
            <h1 style={{ margin: '4px 0', fontSize: 20 }}>คำสั่งซื้อ #{order.trackingNumber || id}</h1>
            <div style={{ fontSize: 13, opacity: 0.9 }}>วันที่สั่งซื้อ: {dateText}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 12, opacity: 0.85 }}>ยอดรวม</div>
            <div style={{ fontSize: 22, fontWeight: 700 }}>฿{totalText}</div>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.3fr', gap: 16, alignItems: 'flex-start' }}>
          {/* Left: items & summary */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: '#fff', borderRadius: 18, padding: 16, boxShadow: '0 4px 12px rgba(15,23,42,0.06)' }}>
              <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>สินค้าในคำสั่งซื้อ</h2>
              {items.length === 0 ? (
                <div style={{ fontSize: 13, color: '#6B7280' }}>ไม่มีสินค้า</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {items.map((it, idx) => (
                    <div
                      key={idx}
                      style={{
                        display: 'flex',
                        justifyContent: 'space-between',
                        fontSize: 13,
                        padding: '6px 0',
                        borderBottom: idx === items.length - 1 ? 'none' : '1px solid #F3F4F6',
                      }}
                    >
                      <div>
                        <div style={{ fontWeight: 500 }}>{it.productName || '-'}</div>
                        <div style={{ color: '#6B7280' }}>จำนวน {it.quantity || 0} ชิ้น</div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ color: '#111827' }}>฿{(it.subtotal || 0).toLocaleString()}</div>
                        <div style={{ color: '#9CA3AF', fontSize: 12 }}>฿{(it.price || 0).toLocaleString()} / ชิ้น</div>
                      </div>
                    </div>
                  ))}
                </div>
              )}
            </div>
          </div>

          {/* Right: shipping info */}
          <div style={{ background: '#fff', borderRadius: 18, padding: 16, boxShadow: '0 4px 12px rgba(15,23,42,0.06)' }}>
            <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>ข้อมูลการจัดส่ง</h2>
            <div style={{ fontSize: 13, color: '#374151' }}>วิธีส่ง: {deliveryMethod}</div>
            {order.requestedAddress && (
              <div style={{ fontSize: 13, color: '#4B5563', marginTop: 6, whiteSpace: 'pre-wrap' }}>
                ที่อยู่จัดส่ง: {order.requestedAddress}
              </div>
            )}
            {order.shippingCarrier && (
              <div style={{ fontSize: 13, color: '#374151', marginTop: 8 }}>
                ขนส่ง: {order.shippingCarrier}
              </div>
            )}
            {order.trackingNumber && (
              <div style={{ fontSize: 13, color: '#374151', marginTop: 4 }}>
                Tracking: <span style={{ fontFamily: 'monospace' }}>{order.trackingNumber}</span>
              </div>
            )}
            <div style={{ fontSize: 13, color: '#374151', marginTop: 10 }}>
              สถานะการจัดส่ง:
            </div>
            <span
              style={{
                display: 'inline-block',
                marginTop: 4,
                padding: '4px 10px',
                borderRadius: 999,
                background: `${statusColor}14`,
                color: statusColor,
                fontSize: 12,
                fontWeight: 600,
              }}
            >
              {status}
            </span>
          </div>
        </div>
      </div>
    </div>
  );
}
